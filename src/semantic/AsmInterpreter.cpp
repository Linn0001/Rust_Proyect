...
void AsmInterpreter::snapshot(const Instr& ins) {
    ...
    // Reconstruir pila lógica desde memByOffset como variables genéricas
    trace.vars.clear();
    trace.nextOffset = 0;
    ...
    for (auto& kv : memByOffset) {
        ...
        // No duplicar el offset 0: lo usamos para RBP_OLD en la UI
        if (offset == 0) {
            continue;
        }
        ...
        std::string name = "off" + std::to_string(offset);

        if (itf != floatSlots.end()) {
            // Sabemos que este slot contiene un double (f64)
            double d = itf->second;
            ...
            // Entero normal (interpretación signed 64 bits)
