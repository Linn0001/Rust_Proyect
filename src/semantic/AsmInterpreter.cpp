...
long long AsmInterpreter::readOperand(const std::string& operand) {
    ...
    // Registro (varios tamaños, pero los tratamos como 64 bits lógicos)
    if (!op.empty() && op[0] == '%') {
...
void AsmInterpreter::snapshot(const Instr& ins) {
    ...
    // Reconstruir pila lógica desde memByOffset
    trace.vars.clear();
    trace.nextOffset = 0;
    for (auto& kv : memByOffset) {
        long long offset = kv.first;
        long long val    = kv.second;
        trace.vars["off" + to_string(offset)] = { offset, val };
        trace.nextOffset = std::min(trace.nextOffset, offset);
    }

    std::string label = "Línea " + std::to_string(ins.lineNumber) + ": " + trim(ins.text);
...
void AsmInterpreter::execute(const Instr& ins, int& ip) {
    ...
    if (op == "movq" || op == "movl" || op == "movw" || op == "movb") {
        ...
        // ================= SETcc (signed / eq / neq) =================
    else if (op == "setg") {      // signed >
        long long L = lastCmpLhs, R = lastCmpRhs;
        writeOperand(a[0], (L > R) ? 1 : 0);
        ++ip;
    }
    else if (op == "setge") {     // signed >=
        long long L = lastCmpLhs, R = lastCmpRhs;
        writeOperand(a[0], (L >= R) ? 1 : 0);
        ++ip;
    }
    else if (op == "setl") {      // signed <
        long long L = lastCmpLhs, R = lastCmpRhs;
        writeOperand(a[0], (L < R) ? 1 : 0);
        ++ip;
    }
    else if (op == "setle") {     // signed <=
        long long L = lastCmpLhs, R = lastCmpRhs;
        writeOperand(a[0], (L <= R) ? 1 : 0);
        ++ip;
    }
        ...
        if (it != labelToIndex.end()) {
            ...
            // printf u otra externa → la ignoramos
            ++ip;
        }
    }
